...
const Menu: React.FC<MenuProps> = ({ onStart, isDarkMode, toggleTheme }) => {
  ...
  return (
    ...
    <div className="min-h-screen lg:h-screen bg-gray-50 dark:bg-gray-950 flex flex-col transition-colors duration-500 relative overflow-x-hidden lg:overflow-hidden">
      ...
      {/* Main Content Area */}
      <div className="flex-grow flex flex-col p-4 lg:p-6 overflow-hidden">
        <div className="max-w-[1920px] w-full mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-12 flex-grow h-full text-center lg:text-left">

          {/* Left Column: Intro & Actions */}
          <div className="flex flex-col justify-center space-y-8 animate-fade-in-up py-4 lg:py-0 overflow-y-auto custom-scrollbar">
          ...
          <div className="w-full h-full bg-white dark:bg-gray-900/80 rounded-[2rem] shadow-2xl border border-gray-200 dark:border-gray-800 overflow-hidden flex flex-col animate-fade-in-up delay-100 relative ring-8 ring-gray-100/50 dark:ring-gray-800/30 backdrop-blur-sm">
            {/* Chat Header */}
            ...
            <div className="flex-grow overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-6 bg-white dark:bg-gray-900">
              {guideMessages.map((msg) => (
                <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  ...
                  <div className={`max-w-[85%] px-4 sm:px-6 py-3 sm:py-4 rounded-2xl text-base sm:text-lg leading-relaxed shadow-sm ${msg.role === 'user'
